...
var ASCIIGame = {
    init: function(el, options) {
        ...
        var game = {
                ...
                player: null,
                playerName: document.getElementById('nameDisp').innerText.replace('Enter your name and press Enter: ', ''),
                lastFrame: 0,
                mainLoop: function() {
                        ...
                        if (new Date() - game.lastFrame > 50 /*ms*/ ) {
                            ...
                            for (var i = 0; i < game.eventData.frame.length; ++i) {
                                var c = tools.unpack(game.eventData.frame[i]),
                                    x = c[0],
                                    y = c[1];
                            ...
                            for (var i = 0; i < game.physicsData.length; ++i) {
                                var c = tools.unpack(game.physicsData[i]),
                                    x = c[0],
                                    y = c[1],
                                    d = data.get(x, y);
                                ...
                                var newX = Math.round(d.physics.positionX),
                                    newY = Math.round(d.physics.positionY);
                                if (x !== newX || y !== newY) {
            ...
            data = {
                ...
                moveWithCollision: function(x1, y1, x2, y2) {
                    ...
                    x2 = c[0];
                    y2 = c[1]; // will be changed to x1/y1 in loop below
                ...
                render: function() {
                    for (var i = 0; i < data.modified.length; ++i) {
                        var c = tools.unpack(data.modified[i]),
                            x = c[0],
                            y = c[1];
        ...
        var asciiGame = {
            play: function() {
                ...
                window.onkeydown = function(e) {
                    if (_firsttime) console.log(e.which, e.keyCode);
                    _firsttime = false;
                    tools.keysDown[e.which || e.keyCode] = true;
                };
                window.onkeyup = function(e) {
                    tools.keysDown[e.which || e.keyCode] = false;
                };
